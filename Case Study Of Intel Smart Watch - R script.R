...
library(tibble) # For better table formatting
...
key_variables <- c("ConstCom", "TimelyInf", "TaskMgm", "DeviceSt", 
...
descriptive_stats <- df %>%
  ...
  t()  # Transpose the table to correct alignment

# ----Convert to a Proper Data Frame ----
descriptive_table <- as.data.frame(descriptive_stats)
colnames(descriptive_table) <- key_variables  # Assign proper variable names
descriptive_table <- tibble::rownames_to_column(descriptive_table, var = "Statistic")
